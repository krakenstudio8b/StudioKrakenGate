...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const openDetail = (task) => {
        ...
        html += `<div class="mt-4 pt-4 border-t flex justify-end">
            <button id="restore-task-btn" class="bg-indigo-500 text-white font-semibold py-2 px-4 rounded-lg hover:bg-indigo-600 text-sm flex items-center gap-2">
                <i class="fa-solid fa-rotate-left"></i> Ripristina alla Board
            </button>
        </div>`;
        ...
        if (restoreBtn) {
            restoreBtn.addEventListener('click', () => restoreTask(currentArchiveTaskId));
        }
    ...
    onValue(tasksRef, (snapshot) => {
        ...
        completedTasks = allTasks.filter(t => t.archived === true);
        updateStats();
        applyFilters();
    });
});
